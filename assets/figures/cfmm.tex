...
\uncover<4->{
	...
	% Token labels
\uncover<2->{
\node at (-0.5,-0.7) {\scriptsize{CLC}};
\node at (0.5,-0.7) {\scriptsize{ETH}};
}

\uncover<3->{
% \node at (-4.5, 0.5) {\scriptsize{Pool Token}};
 \node at (-4.5, 0.5) {\includegraphics[scale = 0.05]{../assets/images/pie-chart}};
}
	...
	\uncover<2>{
		...
		\draw[->] (-3.5, -0.5) -- (-1.5, -0.5) node [midway, above]{\scriptsize{CLC}};
	}
	\uncover<3>{
	\draw[->, darkmint] (-1.5, 0) -- (-3.5, 0) node [midway, above] {\scriptsize{Pool Token}};
	}
	\uncover<4->{
	...
	\uncover<4->{
		\draw[->] (-0.5,0.12 ) arc (150:30:2.5cm) node [midway, above] {\scriptsize{CLC}};
