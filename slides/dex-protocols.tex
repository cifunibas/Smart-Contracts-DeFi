...
\begin{frame}{Slippage (without fees)}
	...
	\uncover<5->{
		\begin{keytakeaway}{Price slippage}
			Price slippage $S$ is the deviation of the price for the actual trade $\overline{P_{x}}$ from the price for an infinitesimal trade $P_x$.
		\end{keytakeaway}
	}
\end{frame}
%%%
...
\begin{frame}{Slippage (with fees)}
	...
	\begin{minipage}{0.35\textwidth}
		...
		\begin{scriptsize}
			...
			\only<4->{
				\intertext{ \textbf{Fee Slippage:} \vspace{-0.2cm}}
...
\begin{frame}{Slippage with fees}
	...
	\begin{align*}
			\onslide<1->{
				...
				S_{price} &= \tfrac{\overline{P_{x}}}{P_x} - 1\\
			}
			\onslide<1->{
				\intertext{ Fee Slippage: }				
				S_\rho &= \tfrac{\overline{P_{x,\rho}}}{\overline{P_{x,}}} - 1\\
			}
			\onslide<1->{
				\intertext{ Total Slippage: }				
				S &= \tfrac{\overline{P_{x,\rho}}}{P_x} - 1 \\
				&= 1 - (1+S_{price}) \cdot (1+S_\rho)\\
			}
	\end{align*}
	\end{minipage}
...
Let us now assume we make the same trade as before but in a larger pool with 100 X-tokens and 100 Y-tokens:
	...
	\begin{minipage}[t]{0.4\textwidth}
		\begin{align*}
			...
			\uncover<6->{
				P'_{x} &= \gamma \cdot \tfrac{y'_\rho}{x'_\rho} = 0.8 \cdot \tfrac{96.1538}{105} = 0.7326 \\
			}
			\uncover<7->{
...
\begin{figure}
	...
	\resizebox{0.8\textwidth}{!}{
	\begin{tikzpicture}[scale=1.0, every node/.style={scale=1.0}]
			\input{../assets/figures/defi_stack_aggregation_layer.tex}
	\end{tikzpicture}}
\end{figure}
