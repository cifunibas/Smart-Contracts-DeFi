...
\begin{frame}{Slippage (without fees)}
	...
	\uncover<5->{
		\begin{keytakeaway}{Price slippage}
			Price slippage $S$ is the deviation of the price for the actual trade $\overline{P_{x}}$ from the price for an infinitesimal trade $P_x$. A smaller trade size compared to the total pool liquidity reduces price slippage.
		\end{keytakeaway}
	}
\end{frame}
%%%
