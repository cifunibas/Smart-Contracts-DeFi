...
\vspace{1.5em}
To be able to send ether to an address, it needs to be explicitly declared as \genkey{payable:}\\
...
\begin{minipage}{0.65\textwidth}
\textbf{As Blockchains are fully transparent, so is the full history of bids?}
...
\uncover<4->{
\textit{No: This is expensive and causes state bloat.}
}
...
\begin{itemize}
	\item	\textbf{Cheap:} It is stored in a separate place than state.
	\item	\textbf{Verifiable:} It is part of the block hash.
	\item	\textbf{Downside:} It can not be accessed on-chain by contracts.
\end{itemize}
}
