...
\begin{frame}{Flash Minting}
	...
	In some cases, protocols allow flash minting of their native token up to a certain \texttt{cap}. \vspace{0.5em}
	
	\begin{itemize}
		\item This loosens the liquidity constraint $LP_x \geq x$. 
		\item Instead, the amount available to borrowers is given by:\\
			\texttt{cap} $\leq$ \texttt{type(uint256).max - totalSupply()}.
	\end{itemize}
