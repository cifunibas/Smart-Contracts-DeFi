...
\uncover<2->{
\begin{alertblock}{Key takeaways}
Every transaction type modifies the data structure. Depending on the order in which transactions are executed the current state of the ledger can be different. Users can explicitly set the order of their own transactions. Conflicting transactions are handled by the network's consensus protocol. 
\end{alertblock}
}
